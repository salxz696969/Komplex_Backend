...
import { followers, users, userSavedBlogs } from "@/db/schema.js";
	...
	try {
		...
		if (searchResults.hits.length === 0) {
			...
			idsFromSearch = Array.from(
				Array.from(new Set([...followedUsersBlogsId.map((f) => f.id), ...blogIds.map((f) => f.id)]))
			);
		}
		...
		return { data: blogsWithMedia, hasMore: allBlogs.length === limit, isMatch: searchResults.hits.length > 0 };
	} catch (error) {
