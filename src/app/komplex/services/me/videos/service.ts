...
export const getAllMyVideos = async (query: any, userId: number) => {
	...
	const cacheKey = `myVideos:${userId}:type:${type || "all"}:topic:${topic || "all"}:page:${page || 1}`;
	...
	await redis.set(cacheKey, JSON.stringify({ data: videoRows, hasMore: videoRows.length === limit }), {
		EX: 60 * 60 * 24,
	});
...
export const getMyVideoHistory = async (userId: number) => {
	...
	await redis.set(cacheKey, JSON.stringify(videoHistory), { EX: 600 });
...
export const postVideo = async (body: any, userId: number) => {
	const { videoKey, title, description, topic, type, thumbnailKey, duration, questions } = body;
	...
	const myVideoKeys: string[] = await redis.keys(`myVideos:${userId}:type:*:topic:*:page:*`);
