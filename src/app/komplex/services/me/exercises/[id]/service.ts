...
export const getExerciseById = async (id: string, userId: number) => {
	...
	for (const exerciseHistoryId of exerciseHistoryIds) {
		const questionsHistory = await db
			...
			.leftJoin(userExerciseHistory, eq(exerciseQuestionHistory.exerciseHistoryId, userExerciseHistory.id))
			.where(and(eq(exerciseQuestionHistory.exerciseHistoryId, exerciseHistoryId.id)));
	...
	const sectionScoresByAttempt = questionHistory.map((attempt) => {
		...
		for (const section of sections) {
			if (sectionStats[section].total > 0) {
				sectionStats[section].score = (sectionStats[section].correct / sectionStats[section].total) * 100;
			}
		}
...
export const submitExercise = async (id: string, answers: any[], score: number, timeTaken: number, userId: number) => {
	try {
		...
		await redis.del(`exerciseHistory:userId:${userId}`);
