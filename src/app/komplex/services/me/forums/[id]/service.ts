...
import { forums, forumMedias, users, forumLikes, forumComments } from "@/db/schema.js";
import { uploadImageToCloudflare, deleteFromCloudflare } from "@/db/cloudflare/cloudflareFunction.js";
...
export const updateForum = async (id: string, body: any, files: any, userId: number) => {
	...
	const myForumKeys: string[] = await redis.keys(`myForums:${userId}:type:*:topic:*`);
...
export const deleteForum = async (id: string, userId: number) => {
	...
	if (mediaToDelete && mediaToDelete.length > 0) {
		await Promise.all(mediaToDelete.map((media) => deleteFromCloudflare("komplex-image", media.urlToDelete ?? "")));
	}
