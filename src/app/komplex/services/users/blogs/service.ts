...
export const getUserBlogs = async (
  ...
  try {
    ...
    const cacheKey = `userBlogs:${userId}:type:${type || "all"}:topic:${
      topic || "all"
    }:page:${pageNumber}`;
    ...
    if (parsedCached) {
      return { data: parsedCached, hasMore: parsedCached.length === limit };
    }
    ...
    // Cache for 5 minutes
    await redis.set(cacheKey, JSON.stringify(blogsWithMedia), {
      EX: 300,
    });

    return { data: blogsWithMedia, hasMore: blogsWithMedia.length === limit };
  } catch (error) {
    throw new Error("Failed to get user blogs" + error);
  }
};
