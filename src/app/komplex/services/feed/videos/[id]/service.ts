...
export const getVideoById = async (videoId: number, userId: number) => {
	...
	if (cacheVideoData && cacheExercisesData) {
		...
		return { data: { ...video, exercises, isFollowing: isFollowing.length > 0 } };
	}
	...
	const isFollowing = await db
		.select()
		.from(followers)
		.where(and(eq(followers.followedId, Number(videoRow.userId)), eq(followers.userId, userId)));

	const videoWithExercises = { ...videoRow, exercises: videoExercises, isFollowing: isFollowing.length > 0 };

	return { data: videoWithExercises };
};

export const getRecommendedVideos = async (userId: number, videoId: number, limit: number, offset: number) => {
	let query = "";
	const cacheVideoKey = `video:${videoId}`;
	const cacheVideoData = await redis.get(cacheVideoKey);
	if (cacheVideoData) {
		...
		await redis.set(cacheVideoKey, JSON.stringify(videoRow), { EX: 600 });
    query = videoRow ? `${videoRow.title} ${videoRow.description} ${videoRow.topic} ${videoRow.type}` : "";
	}
