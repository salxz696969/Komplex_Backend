...
export const getAllVideos = async (type?: string, topic?: string, page?: string, userId?: number) => {
	try {
		const conditions = [];
		...
		if (missedIds.length > 0) {
			const videoRows = await db
				...
				.leftJoin(userSavedVideos, and(eq(userSavedVideos.videoId, videos.id), eq(userSavedVideos.userId, 1)))
				.leftJoin(videoLikes, and(eq(videoLikes.videoId, videos.id), eq(videoLikes.userId, 1)))
		...
		const videoUserIdRows = Array.from(
			Array.from(new Set([...followedUsersVideosId.map((f) => f.userId), ...videoIds.map((f) => f.userId)])).map(
				(id) => ({
					userId: id,
				})
			)
		...
		return { data: videosWithMediaAndIsFollowing, hasMore: allVideos.length === limit };
	} catch (error) {
