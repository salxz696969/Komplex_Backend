...
import { forumComments, forumCommentMedias, users, forumCommentLikes } from "@/db/schema.js";

export const getAllCommentsForAForum = async (id: string, userId: number, page: number = 1) => {
	...
	const dynamicData = await db
		...
		.leftJoin(
			forumCommentLikes,
			and(eq(forumCommentLikes.forumCommentId, forumComments.id), eq(forumCommentLikes.userId, Number(userId)))
		)
