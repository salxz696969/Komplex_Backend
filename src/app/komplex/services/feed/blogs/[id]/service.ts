...
import { blogs, blogMedia, users, userSavedBlogs, followers } from "@/db/schema.js";
...
export const getBlogById = async (id: string, userId: number) => {
	...
	const isFollowing = await db
		.select()
		.from(followers)
		.where(and(eq(followers.followedId, Number(blogData[0].userId)), eq(followers.userId, userId)));
