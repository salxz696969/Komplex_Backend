...
export const getAllBlogs = async (type?: string, topic?: string, page?: string, userId?: number) => {
	try {
		...
		const blogUserIdRows = Array.from(
			Array.from(new Set([...followedUsersBlogsId.map((f) => f.userId), ...blogIds.map((f) => f.userId)])).map(
				(id) => ({
					userId: id,
				})
			)
		...
		return { data: blogsWithMediaAndIsFollowing, hasMore: allBlogs.length === limit };
	} catch (error) {
