...
export const getAllForums = async (type?: string, topic?: string, page?: string, userId?: number) => {
	try {
		const conditions = [];
		...
		const forumIdRows = Array.from(
			new Set([...followedUsersForumIds.map((f) => f.id), ...forumIds.map((f) => f.id)])
		).map((id) => ({ id }));
		...
		const cachedResults = (await redis.mGet(forumIdRows.map((f) => `forums:${f.id}`))) as (string | null)[];
    ...
    const forumUserIdRows = Array.from(
			Array.from(new Set([...followedUsersForumIds.map((f) => f.userId), ...forumIds.map((f) => f.userId)])).map(
				(id) => ({
					userId: id,
				})
			)
		...
		return { data: forumsWithMediaAndIsFollowing, hasMore: allForums.length === limit };
	} catch (error) {
