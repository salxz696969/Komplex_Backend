	...
	try {
		...
		if (!cachedReplies.length) {
			const replyRows = await db
				...
				.leftJoin(videoReplyLike, eq(videoReplies.id, videoReplyLike.videoReplyId))
				.where(eq(videoReplies.videoCommentId, Number(id)))
				.groupBy(
					videoReplies.id,
					videoReplies.userId,
				...
				.orderBy(
					...
					desc(sql`CASE WHEN DATE(${videoReplies.updatedAt}) = CURRENT_DATE THEN 1 ELSE 0 END`),
...
export const postForumVideoReply = async (req: AuthenticatedRequest, res: Response) => {
	try {
		...
		await redis.set(`videoReplies:comment:${id}:lastPage`, JSON.stringify({ currentReplyAmount, lastPage }), {
			EX: 600,
		});
...
export const updateForumVideoReply = async (req: AuthenticatedRequest, res: Response) => {
	try {
		...
		await redis.del(`videoReplies:comment:${updateReply.videoCommentId}:lastPage`);
...
export const deleteVideoReply = async (userId: number, videoReplyId: number | null, commentId: number | null) => {
	...
	if (videoReplyId && commentId === null) {
		...
		await redis.del(`videoReplies:comment:${deletedReply[0]?.videoCommentId}:lastPage`);
