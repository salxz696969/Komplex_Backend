...
import { deleteReply } from "../feed/forum_replies.controller.js";
import { deleteComment } from "../feed/forum_comments.controller.js";
...
export const deleteForum = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    console.log("deleted from cloudflare: ", mediaToDelete);

    const deletedMedia = await db
      .delete(forumMedias)
      .where(eq(forumMedias.forumId, Number(id)));

    console.log("deleted from db: ", deletedMedia);

    // delete from likes
    const deletedLikes = await db
      .delete(forumLikes)
      .where(eq(forumLikes.forumId, Number(id)));

    console.log("deleted from likes: ", deletedLikes);

    // delete from comments and replies
    const commentRecords = await db
      .select()
      .from(forumComments)
      .where(eq(forumComments.forumId, Number(id)));
    console.log("deleted from comments: ", commentRecords);
    ...
    if (commentRecords.length > 0) {
      for (const commentRecord of commentRecords) {
        ...
        console.log("deleted from replies: ", deleteReplies);
      }
      deleteComments = await deleteComment(Number(userId), null, Number(id));
      console.log("deleted from comments: ", deleteComments);
    }
    ...
    const deletedForum = await db
      .delete(forums)
      .where(eq(forums.id, Number(id)));
    console.log("deleted from forums: ", deletedForum);

    console.log("deleted from db: ", deletedForum);
