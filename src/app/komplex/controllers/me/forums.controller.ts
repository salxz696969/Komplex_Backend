...
export const getAllMyForumsController = async (req: AuthenticatedRequest, res: Response) => {
	try {
...
export const postForumController = async (req: AuthenticatedRequest, res: Response) => {
	try {
		const { userId } = req.user ?? { userId: 1 };
		const result = await forumService.postForum(req.body, req.files, Number(userId));
...
export const likeForumController = async (req: AuthenticatedRequest, res: Response) => {
	try {
		const { id } = req.params;
		const { userId } = req.user ?? { userId: 1 };
...
export const unlikeForumController = async (req: AuthenticatedRequest, res: Response) => {
	try {
		const { id } = req.params;
		const { userId } = req.user ?? { userId: 1 };
...
export const updateForumController = async (req: AuthenticatedRequest, res: Response) => {
	try {
		const { userId } = req.user ?? { userId: "1" };
		const { id } = req.params;
		const result = await forumByIdService.updateForum(id, req.body, req.files, Number(userId));
		return res.status(200).json(result);
	} catch (error) {
...
export const deleteForumController = async (req: AuthenticatedRequest, res: Response) => {
	try {
		const { userId } = req.user ?? { userId: "1" };
		const { id } = req.params;
