	...
	try {
		...
		if (redisData) {
			...
			for (const exercise of allExercises) {
				...
				cacheExerciseForAGrade.push({ ...exercise, numberOfQuestions: numberOfQuestions[0].numberOfQuestions });
			}
			await redis.set(cacheKey, JSON.stringify(cacheExerciseForAGrade), { EX: 60 * 60 * 24 });
		}
...
export const getExercise = async (req: Request, res: Response) => {
	try {
		...
		await redis.set(cacheKey, JSON.stringify(exerciseWithQuestions), { EX: 600 });
