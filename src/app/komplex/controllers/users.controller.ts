...
export const getUserFollowers = async (req: AuthenticatedRequest, res: Response) => {
	try {
		...
		await redis.set(cacheKey, JSON.stringify(followersList), { EX: 60 * 60 * 24 });
...
export const followUser = async (req: AuthenticatedRequest, res: Response) => {
	try {
		...
		const redisData = await redis.get(cacheKey);
		if (redisData) {
			await redis.set(cacheKey, JSON.stringify([...JSON.parse(redisData), follow]), { EX: 60 * 60 * 24 });
		} else {
