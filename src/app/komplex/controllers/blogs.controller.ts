...
import { blogs, followers, users, userSavedBlogs } from "../../../db/schema.js";
...
export const getAllBlogs = async (req: AuthenticatedRequest, res: Response) => {
    ...
    const blogIdRows = Array.from(
			Array.from(new Set([...followedUsersBlogsId.map((f) => f.id), ...blogIds.map((f) => f.id)])).map((id) => ({ id }))
		);

		if (!blogIdRows.length) return res.status(200).json({ blogsWithMedia: [], hasMore: false });
