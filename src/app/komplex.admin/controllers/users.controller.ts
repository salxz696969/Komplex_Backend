...
export const getAllUsers = async (req: Request, res: Response) => {
	try {
		...
		const cachedResults = (await redis.mGet(result.map((v) => `users:${v.userId}`))) as (string | null)[];
		const hits: any[] = [];
		...
		if (missedIds.length > 0) {
			const userRows = await db.select().from(users).where(inArray(users.id, missedIds));
