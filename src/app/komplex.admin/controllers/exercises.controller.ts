...
import { choices, exercises, questions, userExerciseHistory } from "../../../db/schema.js";
...
export const getExercises = async (req: Request, res: Response) => {
	try {
		...
		let baseQuery = db
			...
			.leftJoin(userExerciseHistory, eq(exercises.id, userExerciseHistory.exerciseId));

		// Execute query with or without filters
		let result;
		if (grade) {
			result = await baseQuery.where(and(eq(exercises.grade, grade as string))).groupBy(exercises.id);
		} else {
...
export const getExercise = async (req: Request, res: Response) => {
	try {
		...
		await redis.set(cacheKey, JSON.stringify(exerciseWithQuestions), { EX: 60 * 60 * 24 });
...
export const createExercise = async (req: Request, res: Response) => {
	try {
		...
		const { duration, title, description, subject, grade, exerciseQuestions } = req.body;
...
export const getExerciseDashboard = async (req: Request, res: Response) => {
	try {
		...
		const totalScores = await db
			...
			.from(userExerciseHistory);
		const averageScore = totalScores[0].averageScore ? parseFloat(totalScores[0].averageScore) : 0;
		const cacheData = {
			totalExercises,
			totalAttempts,
			averageScore,
