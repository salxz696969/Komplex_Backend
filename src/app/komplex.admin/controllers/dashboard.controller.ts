...
export const getDashboardData = async (req: Request, res: Response) => {
	try {
		...
		// Get content counts
		const totalBlogsResult = await db.select({ count: sql<number>`count(*)` }).from(blogs);
		...
		const totalExercisesResult = await db.select({ count: sql<number>`count(*)` }).from(exercises);
		const totalExercises = Number(totalExercisesResult[0]?.count || 0);

		const totalForumsResult = await db.select({ count: sql<number>`count(*)` }).from(forums);
		const totalForums = Number(totalForumsResult[0]?.count || 0);

		const totalContent = totalBlogs + totalVideos + totalExercises + totalForums;
		...
		const totalLikesResult = await db.select({ count: sql<number>`count(*)` }).from(videoLikes);
		const videoLikesCount = Number(totalLikesResult[0]?.count || 0);

		const totalForumLikesResult = await db.select({ count: sql<number>`count(*)` }).from(forumLikes);
		const forumLikesCount = Number(totalForumLikesResult[0]?.count || 0);
		const totalLikes = videoLikesCount + forumLikesCount;

		const totalCommentsResult = await db.select({ count: sql<number>`count(*)` }).from(forumComments);
		const forumCommentsCount = Number(totalCommentsResult[0]?.count || 0);

		const totalVideoCommentsResult = await db.select({ count: sql<number>`count(*)` }).from(videoComments);
		...
		const totalRepliesResult = await db.select({ count: sql<number>`count(*)` }).from(forumReplies);
		const forumRepliesCount = Number(totalRepliesResult[0]?.count || 0);

		const totalVideoRepliesResult = await db.select({ count: sql<number>`count(*)` }).from(videoReplies);
