...
import { db } from "../../db";
import { blogs, mediaTypeEnum, userSavedBlogs } from "../../db/schema";
...
export const getBlogById = async (req: Request, res: Response) => {
	try {
		...
		if (blogFromDb && blogFromDb.length > 0 && blogFromDb[0]) {
			await db
				.update(blogs)
				.set({ viewCount: (blogFromDb[0]?.viewCount ?? 0) + 1, updatedAt: new Date() })
				.where(eq(blogs.id, Number(id)))
				.returning();
		}
		...
		return res.json(blogWithMedia).status(200);
	} catch (error) {
		return res.status(500).json({
			success: false,
			error: (error as Error).message,
